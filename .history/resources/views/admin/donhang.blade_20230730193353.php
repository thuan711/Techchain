    ...
    <div class="single-product pt-5 mb-150">
		<div class="container">
            <table class='table table-hover table-strip table-bordered'>
                ...
                <th>ID Đơn hàng</th> <th>Thời gian đặt hàng</th> <th>Thời điểm mua</th> <th>Tên người nhận</th><th>Số điện thoại & Địa chỉ</th><th>Trạng thái</th><th>Tổng tiền</th><th class="text-end">Hành động</th>
                </tr>
                @foreach ($dh as $dh)
                <tr><td class="align-middle"> {{$dh->id_dh}} </td>
                    ...
                    <td class="align-middle text-end">               
                        <form action="/admin/donhang/{{$dh->id_dh}}" method="post">
                            <a href="/admin/loaisp/{{$dh->id_loai}}/edit" class="btn btn-primary">Chi tiết</a>            
                            <button onclick="return confirm('Bạn muốn xóa đơn hàng?')" class="btn btn-danger" type="submit">Xóa</button>
                            @csrf  @method('DELETE')
                        </form>
                    </td>
                </tr>
