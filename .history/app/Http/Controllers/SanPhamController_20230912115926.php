...
class SanphamController extends Controller
{
    ...
    public function sort($order, Request $request)
    {
        $perpage = 12;
        $products = Sanpham::orderBy('ten_sp', $order)->paginate($perpage)->withQueryString();
        $loai = DB::table('loai')->get();
        return view('spsort', ['products' => $products,'loai' => $loai]);
    }
    public function sortp($orderp, Request $request)
    {
        $perpage = 12;
        $product = Sanpham::orderBy('gia', $orderp)->paginate($perpage)->withQueryString();
        $loai = DB::table('loai')->get();
        return view('spsortp', ['product' => $product,'loai' => $loai]);
    }
